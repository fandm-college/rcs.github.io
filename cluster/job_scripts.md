...
So in this example, the line `# Now that resources and such are specified actually setup and run software` will be ignored by the system.
```
...
```tip
We generally recommend using the `--mem` directive over the `--mem-per-cpu` directive.  That is,
specifying the amount of memory on a per-node basis instead of a per-cpu basis.
```
...
This will script request a minimum of 2 nodes and a maximum of 4 nodes, a total of 24 CPUs spread across those nodes 2 to 4 nodes, and 16GB of memory per node.  The final line sets a run 
...
2. If you will be running software in parallel on multiple nodes, we recommend trying to distribute the number
   of CPUs roughly equally across nodes
